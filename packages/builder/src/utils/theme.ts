...
export const generateThemeStyles = ({
  ...
  const themeVariables = {
    ...
    // Typography Presets
    ...generateTypographyPresetsVariables(
      typography.presets ?? [],
      breakpoints
    ),
    ...
    ...generatePseudoVariables((pseudoClass) => {
      ...
      return {
        ...
        [`--button-${pseudoClass}-background-color`]:
          button.background?.color?.[pseudoClass],
        [`--button-${pseudoClass}-border-color`]:
          button.borderColor?.[pseudoClass],
        [`--button-${pseudoClass}-border-style`]:
          button.borderStyle?.[pseudoClass],
        ...
        [`--button-${pseudoClass}-box-shadow`]: generateBoxShadow(
          button.boxShadow?.[pseudoClass]
        ),
      };
    }),
    ...
    ...generatePseudoVariables((pseudoClass) => {
      return {
        ...
        [`--input-${pseudoClass}-background-color`]:
          form.input?.backgroundColor?.[pseudoClass],
        [`--input-${pseudoClass}-border-color`]:
          form.input?.borderColor?.[pseudoClass],
        [`--input-${pseudoClass}-border-style`]:
          form.input?.borderStyle?.[pseudoClass],
  ...
  const h1Styles = generateTypographyStyles(
    "h1",
    typography.h1 ?? {},
    breakpoints
  );

  const h2Styles = generateTypographyStyles(
    "h2",
    typography.h2 ?? {},
    breakpoints
  );

  const h3Styles = generateTypographyStyles(
    "h3",
    typography.h3 ?? {},
    breakpoints
  );

  const h4Styles = generateTypographyStyles(
    "h4",
    typography.h4 ?? {},
    breakpoints
  );

  const h5Styles = generateTypographyStyles(
    "h5",
    typography.h5 ?? {},
    breakpoints
  );

  const h6Styles = generateTypographyStyles(
    "h6",
    typography.h6 ?? {},
    breakpoints
  );
  ...
  const buttionStyles = {
    ...
    ...generatePseudoStyle((pseudoClass) => ({
      color:
        button.color?.[pseudoClass] && `var(--button-${pseudoClass}-color)`,
      backgroundColor:
        button.background?.color?.[pseudoClass] &&
        `var(--button-${pseudoClass}-background-color)`,
      borderColor:
        button.borderColor?.[pseudoClass] &&
        `var(--button-${pseudoClass}-border-color)`,
      borderStyle: button.borderStyle?.[pseudoClass]
        ? `var(--button-${pseudoClass}-border-style)`
        : undefined,
      borderWidth:
        button.borderWidth?.[pseudoClass] &&
        `var(--button-${pseudoClass}-border-width)`,
      borderRadius:
        button.borderRadius?.[pseudoClass] &&
        `var(--button-${pseudoClass}-border-radius)`,
      boxShadow:
        button.boxShadow?.[pseudoClass] &&
        `var(--button-${pseudoClass}-box-shadow)`,
    })),
  ...
  const inputStyles = {
    ...
    ...generatePseudoStyle((pseudoClass) => ({
      color:
        form.input?.color?.[pseudoClass] && `var(--input-${pseudoClass}-color)`,
      backgroundColor:
        form.input?.backgroundColor?.[pseudoClass] &&
        `var(--input-${pseudoClass}-background-color)`,
      borderColor:
        form.input?.borderColor?.[pseudoClass] &&
        `var(--input-${pseudoClass}-border-color)`,
      borderStyle: form.input?.borderStyle?.[pseudoClass]
        ? `var(--input-${pseudoClass}-border-style)`
        : undefined,
      borderWidth:
        form.input?.borderWidth?.[pseudoClass] &&
        `var(--input-${pseudoClass}-border-width)`,
      borderRadius:
        form.input?.borderRadius?.[pseudoClass] &&
        `var(--input-${pseudoClass}-border-radius)`,
      boxShadow:
        form.input?.boxShadow?.[pseudoClass] &&
        `var(--input-${pseudoClass}-box-shadow)`,
    })),
  };
...
export const generateColorPresetsVariables = (
  presets: ThemeSettings["color"]["presets"]
) => {
...
export const generateTypographyVariables = ({
  ...
  const responsiveVariables = generateResponsiveVariables(
    breakpoints,
    (breakpoint) => {
      return {
        [`--${prefix}-${breakpoint}-font-family`]:
          value.fontFamily?.[breakpoint],
        [`--${prefix}-${breakpoint}-font-weight`]:
          value.fontWeight?.[breakpoint],
        [`--${prefix}-${breakpoint}-text-transform`]:
          value.textTransform?.[breakpoint],
        [`--${prefix}-${breakpoint}-font-style`]: value.fontStyle?.[breakpoint],
        [`--${prefix}-${breakpoint}-text-decoration`]:
          value.textDecoration?.[breakpoint],
        [`--${prefix}-${breakpoint}-font-size`]: generateUnitValue(
          value.fontSize?.[breakpoint]
        ),
        [`--${prefix}-${breakpoint}-line-height`]: generateUnitValue(
...
export const generateButtonPresetsVariables = (
  ...
  return presets?.reduce((acc, preset) => {
    const presetVariables = generatePseudoVariables((pseudoClass) => ({
      ...
      [`--button-preset-${preset.id}-${pseudoClass}-color`]:
      ...
      [`--button-preset-${preset.id}-${pseudoClass}-border-width`]:
        generateUnitValue(preset.value.borderWidth?.[pseudoClass]),
      [`--button-preset-${preset.id}-${pseudoClass}-border-radius`]:
        generateUnitValue(preset.value.borderRadius?.[pseudoClass]),
      [`--button-preset-${preset.id}-${pseudoClass}-box-shadow`]:
        generateBoxShadow(preset.value.boxShadow?.[pseudoClass]),
    }));
...
export const generatePseudoVariables = (
...
export const generateBreakpointVariables = (
  ...
  breakpoints.forEach((breakpoint) => {
    if (breakpoint.maxWidth) {
      variables[`--breakpoint-${breakpoint.key}-max-width`] =
        `${breakpoint.maxWidth}px`;
    }
    if (breakpoint.minWidth) {
      variables[`--breakpoint-${breakpoint.key}-min-width`] =
